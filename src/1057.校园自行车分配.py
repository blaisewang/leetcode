#
# @lc app=leetcode.cn id=1057 lang=python3
#
# [1057] 校园自行车分配
#
# https://leetcode-cn.com/problems/campus-bikes/description/
#
# algorithms
# Medium (42.51%)
# Likes:    45
...
# ，并将其中的自行车分配給工人。如果有多个 (worker, bike)
    ...
    def assignBikes(self, workers: List[List[int]], bikes: List[List[int]]) -> List[int]:
        ...
        for i, (x, y) in enumerate(workers):
            for j, (xb, yb) in enumerate(bikes):
                d.append((abs(x - xb) + abs(y - yb), i, j))

        d.sort()
        s = set()
        r = [-1] * len(workers)
        for dis, w, b in d:
            if r[w] == -1 and b not in s:
                r[w] = b
                s.add(b)
