...
# [642] 设计搜索自动补全系统
#
# https://leetcode-cn.com/problems/design-search-autocomplete-system/description/
#
# algorithms
# Hard (48.23%)
# Likes:    67
...
#
# 为搜索引擎设计一个搜索自动补全系统。用户会输入一条语句（最少包含一个字母，以特殊字符 '#' 结尾）。除 '#'
# 以外用户输入的每个字符，返回历史中热度前三并以当前输入部分为前缀的句子。下面是详细规则：
# 
# 
# 一条句子的热度定义为历史上用户输入这个句子的总次数。
# 返回前三的句子需要按照热度从高到低排序（第一个是最热门的）。如果有多条热度相同的句子，请按照 ASCII 码的顺序输出（ASCII
# 码越小排名越前）。
...
# 
# List<String> input(char c): 其中 c 是用户输入的下一个字符。字符只会是小写英文字母（'a' 到 'z' ），空格（'
# '）和特殊字符（'#'）。输出历史热度前三的具有相同前缀的句子。
# 
# 
# 
# 样例 ：
# 操作 ： AutocompleteSystem(["i love you", "island","ironman", "i love
# leetcode"], [5,3,2,2])
# 系统记录下所有的句子和出现的次数：
# "i love you" : 5 次
# "island" : 3 次
...
# 'r' 的 ASCII 码是 114，所以 "i love leetcode" 在 "ironman" 前面。同时我们只输出前三的句子，所以
# "ironman" 被舍弃。
# 
# 输入 ： input(' ')
...
class AutocompleteSystem:
    ...
    def __init__(self, sentences: List[str], times: List[int]):
        ...
        for x, count in zip(sentences, times):
            d = self.dict
            for s in x:
                d = d[s]
                d[0].append([-count, x])
    ...
    def input(self, c: str) -> List[str]:
        if c == "#":
            ...
            for s in self.word:
                d = d[s]
                if d[0] and self.word == d[0][0][1]:
                    d[0][0][0] = count
                elif len(d[0]) > 1 and self.word == d[0][1][1]:
                    d[0][1][0] = count
                elif len(d[0]) == 3 and self.word == d[0][2][1]:
                    d[0][2][0] = count
                else:
                    d[0].append([count, self.word])
                d[0].sort()
                d[0] = d[0][:3]
            self.word = ""
            return []
