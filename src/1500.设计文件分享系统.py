#
# @lc app=leetcode.cn id=1500 lang=python3
#
# [1500] 设计文件分享系统
#
# https://leetcode-cn.com/problems/design-a-file-sharing-system/description/
#
# algorithms
# Medium (31.84%)
# Likes:    5
...
# 
# 当用户加入系统时，系统应为其注册一个独有的 ID。这个独有的 ID 应当被相应的用户使用一次，但是当用户离开系统时，其 ID
# 应可以被（后续新注册的用户）再次使用。
# 
...
# int join(int[] ownedChunks)：一个新用户加入系统，并拥有文件的一些文件块。系统应当为该用户注册一个 ID，该 ID
# 应是未被其他用户占用的最小正整数。返回注册的 ID。
# void leave(int userID)：ID 为 userID 的用户将离开系统，你不能再从该用户提取文件块了。
# int[] request(int userID, int chunkID)：ID 为 userID 的用户请求编号为 chunkID
...
# 
# fileSharing.request(1, 3);   // id = 1 的用户请求第 3 个文件块，只有 id = 2 的用户拥有文件块，返回
# [2] 。注意，现在用户 1 现拥有文件块 [1,2,3]。
# 
# fileSharing.request(2, 2);   // id = 2 的用户请求第 2 个文件块，id 为 [1,2]
# 的用户拥有该文件块，所以我们返回 [1,2] 。
# 
...
# 
# fileSharing.join([]);        // 一个不拥有任何文件块的用户加入系统，为其注册 id = 1 并返回 1 。注意，id 1
# 和 2 空闲，可以重新使用。
# 
# 
# 
...
class FileSharing:
    ...
    def join(self, ownedChunks: List[int]) -> int:
        ...
        for ch in ownedChunks:
            self.c2i.setdefault(ch, set()).add(ci)

        self.i2c[ci] = set(ownedChunks)
        return ci

    def leave(self, userID: int) -> None:
        if userID in self.i2c:
            for ch in self.i2c[userID]:
                if ch in self.c2i:
                    self.c2i[ch].remove(userID)

            self.i2c.pop(userID)
        self.h.put(userID)

    def request(self, userID: int, chunkID: int) -> List[int]:
        if chunkID not in self.c2i or len(self.c2i[chunkID]) == 0 or userID not in self.i2c:
            return []

        r = sorted([i for i in self.c2i[chunkID]])
        self.c2i[chunkID].add(userID)
        self.i2c[userID].add(chunkID)
