...
# [1236] 网络爬虫
#
# https://leetcode-cn.com/problems/web-crawler/description/
#
# algorithms
# Medium (55.40%)
# Likes:    14
...
#
# 给定一个链接 startUrl 和一个接口 HtmlParser ，请你实现一个网络爬虫，以实现爬取同 startUrl 拥有相同 域名标签
# 的全部链接。该爬虫得到的全部链接可以 任何顺序 返回结果。
# 
# 你的网络爬虫应当按照如下模式工作：
# 
# 
...
# 
# 
# 如上所示的一个链接，其域名为 example.org。简单起见，你可以假设所有的链接都采用 http协议 并没有指定 端口。例如，链接
# http://leetcode.com/problems 和 http://leetcode.com/contest
# 是同一个域名下的，而链接http://example.org/test 和 http://example.com/abc 是不在同一域名下的。
# 
# HtmlParser 接口定义如下： 
# 
...
class Solution:
    def crawl(self, startUrl: str, htmlParser: 'HtmlParser') -> List[str]:
        ...
        for url in rl:
            url = htmlParser.getUrls(url)
            for u in url:
                if u.startswith(host_name) and u not in rl:
                    rs.add(u)
                    rl.append(u)
