...
# [755] 倒水
#
# https://leetcode-cn.com/problems/pour-water/description/
#
# algorithms
# Medium (49.83%)
# Likes:    14
...
# ⁠0123456    
# 
# 当向左或向右移动时，水可以移动到相同或更低的高度。When moving left or right, the water can only move
        ...
        for _ in range(V):
            for d in (-1, 1):
                ...
                while 0 <= i + d < len(heights) and heights[i + d] <= heights[i]:
                    if heights[i + d] < heights[i]: b = i + d
                    i += d
                if b != K:
                    heights[b] += 1
                    break
