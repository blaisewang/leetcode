...
# [1429] 第一个唯一数字
#
# https://leetcode-cn.com/problems/first-unique-number/description/
#
# algorithms
# Medium (48.51%)
# Likes:    6
...
# int showFirstUnique() 返回队列中的 第一个唯一 整数的值。如果没有唯一整数，返回
...
class FirstUnique:
    ...
    def __init__(self, nums: List[int]):
        ...
        self.sn = deque(key for key in self.c.keys() if self.c[key] == 1)
        self.sc = Counter(self.sn)
    ...
    def add(self, value: int) -> None:
        if value not in self.c.keys():
            self.sn.append(value)
            self.sc[value] = 1
        elif value in self.sc.keys():
            self.sn.remove(value)
            del self.sc[value]
