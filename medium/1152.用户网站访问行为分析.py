#
# @lc app=leetcode.cn id=1152 lang=python3
#
# [1152] 用户网站访问行为分析
#
# https://leetcode-cn.com/problems/analyze-user-website-visit-pattern/description/
#
# algorithms
# Medium (36.28%)
# Likes:    6
...
# 页面路径 website。第 i 条记录意味着用户名是 username[i] 的用户在 timestamp[i] 的时候访问了路径为
# website[i] 的页面。
# 
# 我们需要找到用户访问网站时的 『共性行为路径』，也就是有最多的用户都 至少按某种次序访问过一次 的三个页面路径。需要注意的是，用户 可能不是连续访问
# 这三个路径的。
# 
...
class Solution:
    def mostVisitedPattern(self, username: List[str], timestamp: List[int], website: List[str]) -> List[str]:
        d, r = defaultdict(list), defaultdict(set)
        ...
        for i, v in d.items():
            for j in combinations(v, 3):
                r[j].add(i)

        return sorted(r.items(), key=lambda a: (-len(a[1]), a[0]))[0][0]
